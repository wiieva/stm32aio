...
BME280_RESULT BME280_Init ()
{
    ...
    I2Cx_Init(BME280_I2C_PORT,100000);

    DelayMs (10);

    if (!I2Cx_IsDeviceReady (BME280_I2C_PORT,BME280_ADDR,10))
        return BME280_ERROR;
...
BME280_RESULT BME280_Read_Calibration(void) {
    ...
    if (I2Cx_Write(BME280_I2C_PORT,&buf[0],1,BME280_ADDR,I2C_NOSTOP) != SUCCESS) return BME280_ERROR;
    if (I2Cx_Read(BME280_I2C_PORT,(uint8_t *)&cal_param,24,BME280_ADDR) != SUCCESS) return BME280_ERROR;
    ...
    if (I2Cx_Write(BME280_I2C_PORT,&buf[0],1,BME280_ADDR,I2C_NOSTOP) != SUCCESS) return BME280_ERROR;
    if (I2Cx_Read(BME280_I2C_PORT,buf,7,BME280_ADDR) != SUCCESS) return BME280_ERROR;
